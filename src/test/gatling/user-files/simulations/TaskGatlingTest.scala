...
class TaskGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Task entity")
        ...
        .repeat(2) {
            ...
            .get("/interactions/api/tasks")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new task")
            .post("/interactions/api/tasks")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "action":"SAMPLE_TEXT", "deadline":"2020-01-01T00:00:00.000Z", "user":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_task_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created task")
                .get("/interactions${new_task_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created task")
            .delete("/interactions${new_task_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
